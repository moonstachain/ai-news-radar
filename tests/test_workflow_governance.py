...
def test_publication_preflights_actions_pr_permission():
    text = workflow_text()
    assert "Verify PR publication permission" in text
    assert "actions/permissions/workflow" in text
    assert ".can_approve_pull_request_reviews" in text
    assert 'if [ "$CAN_CREATE_PR" != "true" ]' in text
...
def test_publication_never_pushes_the_default_branch_directly():
    ...
    assert push_lines == ['git push --set-upstream origin "$BRANCH"']
